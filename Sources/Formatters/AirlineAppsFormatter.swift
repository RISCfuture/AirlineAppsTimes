...
struct AirlineAppsFormatter: Formatter {
    ...
    func format(_ entry: TimeEntry) -> String {
        ...
        let formattedDualGiven = Self.timeFormatter.string(from: .init(floatLiteral: entry.dualGiven)) ?? Self.defaultFormatted
        let formattedDualReceived = Self.timeFormatter.string(from: .init(floatLiteral: entry.dualReceived)) ?? Self.defaultFormatted
        let formattedTotal = Self.timeFormatter.string(from: .init(floatLiteral: entry.total)) ?? Self.defaultFormatted
